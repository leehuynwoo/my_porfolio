...
$(document).ready(function () {
  function checkWidth() {
    ...
    if (windowSize <= 905) {
      overViewText.text(
        "10대 여성의 피부 트러블이 시작되는 원인은 성호르몬의 변화로 피지선이 자극되기 때문이죠. 이 시기의 여성들은 외모에 민감한 나이로 트러블 피부로 인한 삶의 질 저하를 경험합니다. RED HEAL은 10대 후반의 여성의 피부 트러블에 특화된 솔루션을 제공합니다."
      );
    }
    if (windowSize <= 419) {
      $("#hello h1.section_tit").html(
        "안녕하세요.<br> <span>신입 웹퍼블리셔<br> '이현우'</span>를 소개합니다 "
      );
    }
  }
  checkWidth();
...
//어바웃미 각각 요소 높이 구하기
const aboutmeTitleHegiht = document.querySelector(
...
//img drag and scroll
const slider = document.querySelector(".utilized__img");
let mouseDown = false;
